...
uint8_t broadcastAddress[] = {0x78, 0x21, 0x84, 0x91, 0x30, 0xdc};

// Structure to send data
typedef struct struct_message_send {
  float T;                  // Temperature
  float h;                  // Humidity
  char lightCondition[32];  // Light condition
  bool fanStatus;           // Fan state (true = ON, false = OFF)
  bool waterPumpStatus;     // Water pump state (true = ON, false = OFF)
} struct_message_send;

// Global variable to store the received 2-byte data
uint8_t receivedData[2] = {0, 0};  // Stores the 2-byte data
...
void onDataReceive(const esp_now_recv_info_t *recvInfo, const uint8_t *incomingData, int len) {
  ...
  if (len == 2) {
    receivedData[0] = incomingData[0];  // Store the first byte
    receivedData[1] = incomingData[1];  // Store the second byte
  }

  // Print the received data (without MAC address)
...
void setup()
{
  ...
  //sp_now_register_recv_cb(onDataReceive);
...
void loop()
{
  ...
  if ((receivedData[0] == 0xFA && receivedData[1] == 0x11) || (receivedData[0] == 0x11 && receivedData[1] == 0xFA)) {  // Hex code for Turn on fan
        Serial.println("Fan is turned on ");
    
        fanStatus = true; // Toggle the fan status
  ...
  } else if ((receivedData[0] == 0xFA && receivedData[1] == 0x00) || (receivedData[0] == 0x00 && receivedData[1] == 0xFA)) {  // Hex code for Turn off fan
        Serial.println("Fan is turned off ");
    
        fanStatus = false; // Toggle the fan status
        digitalWrite(FAN_LED_PIN, fanStatus ? HIGH : LOW);
  ...
  } else if ((receivedData[0] == 0xAA && receivedData[1] == 0x11) || (receivedData[0] == 0x11 && receivedData[1] == 0xAA)) {  // Hex code for Turn on water pump
        Serial.println("Water pump is turned on ");
   
        waterPumpStatus = true; // Toggle the water pump status
  ...
  } else if ((receivedData[0] == 0xAA && receivedData[1] == 0x00) || (receivedData[0] == 0x00 && receivedData[1] == 0xAA)) {  // Hex code for Turn off water pump
         Serial.println("Water pump is turned off ");

        waterPumpStatus = false; // Toggle the water pump status
        digitalWrite(WATER_PUMP_LED_PIN, waterPumpStatus ? HIGH : LOW);
  ...
  } else if ((receivedData[0] == 0xFA && receivedData[1] == 0xAA) || (receivedData[1] == 0xAA && receivedData[0] == 0xFA)) {  // Hex code for Turn on light
    Serial.println("Light is turned on ");
    //digitalWrite(LIGHT, HIGH);

  } else if ((receivedData[0] == 0xFA && receivedData[1] == 0xBB) || (receivedData[1] == 0xBB && receivedData[0] == 0xFA)) {  // Hex code for Turn off light
    ...
    sendData.T = T;     // Simulated temperature

    sendData.h = dht.readHumidity();      // Simulated humidity

    strcpy(sendData.lightCondition, lightState == HIGH ? "Dark" : "Light");

    sendData.fanStatus = fanStatus;

    sendData.waterPumpStatus = waterPumpStatus;
  ...
  if (result == ESP_OK) {
    ...
    Serial.println(sendData.h);
    Serial.print("Light Condition: ");
    Serial.println(sendData.lightCondition);
    Serial.print("Fan Status: ");
    Serial.println(sendData.fanStatus ? "ON" : "OFF");
    Serial.print("Water Pump Status: ");
    Serial.println(sendData.waterPumpStatus ? "ON" : "OFF");
  } else {
